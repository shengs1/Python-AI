...
while True:
    n = input("Nhập số nguyên dương (hoặc nhập 'e' để thoát): ")
    if n.lower() == "e":
...
list_of_lists = [[key, value] for key, value in cars.items()]
print("a. Danh sách các danh sách:", list_of_lists)

# b. Chuyển đổi ngược lại từ danh sách các danh sách thành từ điển
restored_dict = dict(list_of_lists)
print("b. Từ điển khôi phục lại:", restored_dict)


print("\n2.Trò chơi bảng cửu chương")
while True:
    try:
        so = int(input("Nhập một số từ 1 đến 10 (nhập 0 để thoát): "))
        if so == 0:
        ...
        bang_cuu_chuong = {}
        for i in range(1, 11):
            bang_cuu_chuong[i] = i * so

        print(f"Bảng cửu chương của {so}:")
        for i, ket_qua in bang_cuu_chuong.items():
            print(f"{i} x {so} = {ket_qua}")
    except ValueError:
...
spices_herbs = [
    "húng quế", "quế", "cam thảo", "bạc hà", "hương thảo", "húng tây", "thảo mộc",
    "nghệ", "rau mùi", "kinh giới cay", "ớt", "ớt", "thì là", "ớt cayenne", "gừng",
    "tỏi", "kinh giới cay", "nhục đậu khấu", "xô thơm", "nghệ tây", "đại hồi", "lá nguyệt quế"]

print("\nCau A")
length_dict = {}
for word in spices_herbs:
    word_length = len(word)
    if word_length not in length_dict:
        length_dict[word_length] = []
    length_dict[word_length].append(word)

print("Từ điển độ dài từ → danh sách từ:")
for k in sorted(length_dict):
    print(f"{k}: {length_dict[k]}")

print("\nCau B")
label_count = {length: len(words) for length, words in length_dict.items()}
print("Từ điển độ dài từ → số lượng nhãn:")
for k in sorted(label_count):
    print(f"{k}: {label_count[k]}")

print("\nCau C")
max_labels = max(label_count.values())
most_common_lengths = [length for length, count in label_count.items() if count == max_labels]

print("Nhãn phổ biến nhất có độ dài:")
for length in most_common_lengths:
    print(f"- {length} ký tự ({max_labels} nhãn): {length_dict[length]}")
